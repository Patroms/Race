	...
	public partial class RaceGame : Form
	{
		...
		private void timerTowardCars_Tick(object sender, EventArgs e)
		{
			...
			if (towardCar3.Top > Height)
			{
				...
				towardCar3.Left = random.Next(0, Width - towardCar3.Width);
			}
		...
		private void GameOver()
		{
			...
			if (coins < 15)
			{
				DialogResult dialogMessage = MessageBox.Show("Game Over!", "Приехали!");
				panelPause.Show();
				panelMenu.Show();
			}
			else
			{
				DialogResult dialogMessage = MessageBox.Show("Продолжить? (-15 coins)", "Приехали!",
